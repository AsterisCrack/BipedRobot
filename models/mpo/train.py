...
class MPOTrainerLSTM(Trainer):
    def __init__(self, env, model_sizes=[[256, 256], [256, 256]], num_workers=1, seq_length=2, device=torch.device("cpu"), seed=42, test_environment=None, steps=int(1e7), epoch_steps=int(5e3), save_steps=int(5e3), test_episodes=5, show_progress=True, replace_checkpoint=False, log=True, log_dir=None, log_name=None, chekpoint_path=None):
